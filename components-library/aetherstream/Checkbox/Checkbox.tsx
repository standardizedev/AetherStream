  ...
  return (
    <label className={`inline-flex items-center gap-3 cursor-pointer ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
      <input
        ...
        disabled={disabled}
      />
      <div className={`relative w-5 h-5 flex-shrink-0 border-2 rounded-sm transition-colors duration-200 
        ${checked ? 'bg-interactive-default border-interactive-default' : 'bg-surface border-border'}
        peer-focus:ring-2 peer-focus:ring-offset-2 peer-focus:ring-offset-surface peer-focus:ring-focus-ring
      `}>
        {checked && (
            <CheckIcon className="w-4 h-4 text-aether-black" />
        )}
      </div>
