  ...
  return (
    <div className="relative w-full" ref={wrapperRef}>
      <button
        ...
        className="w-full flex items-center justify-between px-3 py-2 text-left bg-surface border border-border rounded-sm focus:outline-none focus:ring-1 focus:ring-inset focus:ring-focus-ring"
      >
        <div className="flex flex-wrap gap-1.5 flex-1">
          {selectedOptions.length === 0 ? (
            ...
            selectedOptions.map((option) => (
              <span key={option} className="flex items-center gap-1.5 bg-interactive-default/20 text-interactive-default text-xs font-semibold px-2 py-0.5 rounded-sm uppercase">
                {option}
      ...
      {isOpen && (
        <div className="absolute z-10 w-full mt-1 bg-surface border border-border rounded-sm shadow-lg max-h-60 overflow-auto">
          <ul>
            {options.map((option) => (
              <li
                key={option}
